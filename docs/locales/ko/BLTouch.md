...
* Bltouch V3.0 은 5V 나 OPEN-DRAIN 아웃풋 모드 셋팅을 지원한다. Bltouch V3.1 은 이 두가지를 지원한다. 그러나 또한 내부 EEPROM 내에 이것을 저장할수도 있다. 만일 컨트롤러 보드가 5V 모드의 고정된 5V 하이로직 레벨이 필요하다면, 프린터 설정파일의 [bltouch]섹션에 있는 'set_output_mode' 파라메터의 값을 "5V" 로 셋팅하면 된다.

   *** 오직 컨트롤러 보드의 입력 라인이 5V tolerant 일때 5V 모드를 사용하라. 이는 Bltouch 버전의 기본 설정이 OPEN-DRAIN 모드이기 때문이다. 잠재적으로 당신의 큰트롤러 보드 CPU를 손상시킬지도 모른다. ***
