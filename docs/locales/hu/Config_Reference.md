...
Örvényáramú induktív szondák támogatása. Meghatározható ez a szakasz (szonda szakasz helyett), hogy engedélyezd ezt a szondát. További információkért lásd a [parancshivatkozást](G-Codes.md#probe_eddy_current).
...
Támogatja a derékszögű és hibrid_corexy/z nyomtatókat, amelyek egy tengelyen vannak két kocsival. A kocsi mód a SET_DUAL_CARRIAGE kiterjesztett G-kód paranccsal állítható be. Például a "SET_DUAL_CARRIAGE CARRIAGE=1" parancs aktiválja az ebben a szakaszban meghatározott kocsit (CARRIAGE=0 visszaadja az aktiválást az elsődleges kocsinak). A kettős kocsi támogatását általában extra extruderekkel kombinálják – a SET_DUAL_CARRIAGE parancsot gyakran az ACTIVATE_EXTRUDER paranccsal egy időben hívják. Ügyelj arra, hogy a kocsikat leállítsd a deaktiválás alatt. Ne feledd, hogy a G28 parancs során általában először az elsődleges kocsit küldi kezdőpontra, majd ezt követi a `[dual_carriage]' konfigurációs részben meghatározott kocsi. Azonban a `[dual_carriage]` kocsi indul először kezdőpontra, ha mindkét kocsi pozitív irányba indul kezdőpontra, és a [dual_carriage] kocsi `position_endstop` nagyobb, mint az elsődleges kocsié, vagy ha mindkét kocsi kezdőpont felé negatív irányban, és a ` A [dual_carriage]` kocsi `position_endstop` értéke kisebb, mint az elsődleges kocsié.

Ezenkívül használhatod a "SET_DUAL_CARRIAGE CARRIAGE=1 MODE=COPY" vagy a "SET_DUAL_CARRIAGE CARRIAGE=1 MODE=MIRROR" parancsokat a kettős kocsi másolási vagy tükrözési módjának aktiválásához, amely esetben a kocsi megfelelően követi a 0 kocsi mozgását. Ezekkel a parancsokkal egyszerre két rész nyomtatható – vagy két azonos rész (MÁSOLÁS módban), vagy tükrözött rész (MIRROR módban). Vedd figyelembe, hogy a MÁSOLÁS és a TÜKRÖZÉS módok az extruder megfelelő konfigurációját is megkövetelik a kettős kocsin, ami általában a "SYNC_EXTRUDER_MOTION MOTION_QUEUE=extruder EXTRUDER=<dual_carriage_extruder>" vagy hasonló paranccsal érhető el.
...
Jelenti a szonda tekercs hőmérsékletét. Az örvényáram alapú szondákhoz opcionális termikus eltolás kalibrációt tartalmaz. A `[temperature_probe]` szakasz összekapcsolható egy `[probe_eddy_current]`-el, ha mindkét szakaszhoz ugyanazt az utójavítást használod.
...
BMP180/BMP280/BME280/BMP388/BME680 kétvezetékes interfész (I2C) környezeti érzékelők. Vedd figyelembe, hogy ezeket az érzékelőket nem extruderekkel és fűthető ágyakkal való használatra tervezték, hanem inkább a környezeti hőmérséklet (C), a nyomás (hPa), a relatív páratartalom és a BME680 gázszint figyelésére szolgálnak. A [sample-macros.cfg](../config/sample-macros.cfg) oldalon találsz egy gcode_macro-t, amely a hőmérsékleten kívül a légnyomás és a páratartalom jelentésére is használható.
...
AHT10/AHT20/AHT21 kétvezetékes interfész (I2C) környezeti érzékelők. Veddfigyelembe, hogy ezeket az érzékelőket nem extruderekkel és fűtőágyakkal való használatra tervezték, hanem inkább a környezeti hőmérséklet (C) és a relatív páratartalom figyelésére. A [sample-macros.cfg](../config/sample-macros.cfg) oldalon találsz egy gcode_macro-t, amely a hőmérséklet mellett a páratartalom jelentésére is használható.

```
sensor_type: AHT10
#       Használd az AHT10-et is az AHT20 és AHT21 érzékelőkhöz.
#i2c_address:
#       Az alapértelmezett 56 (0x38).
#       Egyes AHT10 érzékelők lehetőséget adnak az 57 (0x39)
#       használatára egy ellenállás mozgatásával.
#i2c_mcu:
#i2c_bus:
#i2c_speed:
#       A fenti paraméterek leírását lásd a "általános I2C beállítások" részben.
#aht10_report_time:
#       A leolvasások közötti intervallum másodpercben.
#       Az alapértelmezett 30, a minimum az 5
```
...
```
sensor_type:
#       Legyen "HTU21D", "SI7013", "SI7020", "SI7021" vagy "SHT21"
#i2c_address:
...
#       A fenti paraméterek leírását lásd a "általános I2C beállítások" részben.
#htu21d_hold_master:
#       Ha az érzékelő képes megtartani az I2C buf-et olvasás közben.
#       Ha true, az olvasás közben más buszkommunikáció nem hajtható végre.
#       Az alapértelmezett érték False.
#htu21d_resolution:
#       A hőmérséklet és páratartalom leolvasásának felbontása.
#       Az érvényes értékek:
...
SHT3X család két vezetékes interfész (I2C) környezeti érzékelő. Ezek az érzékelők -55~125 C hatótávolságúak, így használhatók pl. kamra hőmérséklet figyelésre. Egyszerű ventilátor vagy fűtő vezérlőként is működhetnek.
...
#       van konfigurálva (általában jumperekkel vagy vezetékekkel).
#i2c_mcu:
#i2c_bus:
#i2c_software_scl_pin:
#i2c_software_sda_pin:
...
#lm75_report_time:
#       A leolvasások közötti intervallum másodpercben.
#       Az alapértelmezett érték 0,8, minimum érték a 0,5.
```
...
### Kombinált hőmérséklet érzékelő

A kombinált hőmérséklet-érzékelő egy virtuális hőmérséklet-érzékelő, amely számos más érzékelőn alapul. Ez az érzékelő használható extruderekkel, heter_generic és fűtött ágyakkal.

```
sensor_type: temperature_combined
#sensor_list:
#       Biztosítani kell. Az új "virtuális" érzékelővel kombinálandó érzékelők
#       listája. Például. "temperature_sensor sensor1, extruder, heater_bed"
#combination_method:
#       Biztosítani kell. Az érzékelőhöz használt kombinációs módszer.
#       A rendelkezésre álló lehetőségek a következők: 'max', 'min', 'mean'.
#maximum_deviation:
#       Biztosítani kell. Az egyesítendő érzékelők közötti maximális
#       megengedett eltérés (pl. 5 fok).
#       A letiltásához használj magas értéket (pl. 999,9)
```
...
#heater: extruder
#       A ventilátorhoz társított fűtést meghatározó konfigurációs
#       szakasz neve. Ha itt megadod a fűtőelemek vesszővel
#       elválasztott nevét, akkor a ventilátor engedélyezve lesz,
#       ha valamelyik adott fűtőtest engedélyezve van.
#       Az alapértelmezett az "extruder".
#heater_temp: 50,0
#       Egy hőmérséklet (Celsiusban), amely alá a fűtőelemnek
#       le kell süllyednie, mielőtt a ventilátort letiltják.
#       Az alapértelmezett 50 Celsius.
#fan_speed: 1.0
#       A ventilátor sebessége (0,0 és 1,0 közötti értékként kifejezve),
#       amelyre a ventilátor be lesz állítva, amikor a hozzá tartozó
#       fűtőberendezés engedélyezve van.
...
#initial_BLUE: 0.0
#initial_WHITE: 0.0
#       Lásd a "LED" részt a paraméterekkel kapcsolatos információkért.
```
...
#       buszhoz csatlakoztatva, akkor megadhatod az "óra"
#       (scl_pin) és "data" (sda_pin) érintkezőket.
#       Az alapértelmezés a hardveres I2C használata.
#color_order: RGBW
#       Állítsd be a LED pixelsorrendjét (R, G, B, W betűket
#       tartalmazó sztring használatával).
#       Az alapértelmezett az RGBW.
#initial_RED: 0.0
#initial_GREEN: 0.0
#initial_BLUE: 0.0
#initial_WHITE: 0.0
...
Futásidőben konfigurálható kimeneti tűk dinamikus pwm-ciklus időzítéssel (tetszőleges számú szakasz definiálható "pwm_cycle_time" előtaggal). Az itt konfigurált tűk kimeneti tűkként lesznek beállítva, és futásidőben a "SET_PIN PIN=my_pin VALUE=.1 CYCLE_TIME=0.100" típusú kiterjesztett [g-kód parancsok](G-Codes.md#pwm_cycle_time) segítségével módosíthatók.
...
Konfigurálj egy TMC2240 léptetőmotor-vezérlőt SPI buszon vagy UART-on keresztül. A funkció használatához adj meg egy konfigurációs részt egy "tmc2240" előtaggal, amelyet a megfelelő léptető konfigurációs szakasz neve követ (például "[tmc2240 stepper_x]").
...
#       legnagyobb ellenállás, a 0,0 pedig a legkisebb ellenállás. A tartomány
#       azonban módosítható a „scale” paraméterrel (lásd alább).
...
#       Ha meg van adva, akkor a „wiper_x” paraméternek 0,0 és „skála” között
#       kell lennie. Ez akkor lehet hasznos, ha az MCP4451-et a léptető
#       feszültségreferenciák beállítására használjuk. A 'scale' beállítható az
#       egyenértékű léptető áramerősségre, ha az MCP4451 a legnagyobb
#       ellenálláson volt, majd a 'wiper_x' paraméterek megadhatók a
#       léptető kívánt áramerősségének használatával.
#       Az alapértelmezés az, hogy nem méretezi a „wiper_x” paramétereket.
```
...
#       paraméterrel (lásd alább). Ha egy csatorna nincs megadva, akkor
#       az konfigurálatlanul marad.
#scale:
#       Ez a paraméter használható a 'channel_x' paraméterek
#       értelmezésének megváltoztatására. Ha meg van adva, akkor a
#       'channel_x' paraméternek 0,0 és 'scale' között kell lennie.
#       Ez akkor lehet hasznos, ha az MCP4728-at a léptető
#       feszültségreferenciák beállítására használjuk. A 'scale' beállítható
#       az egyenértékű léptető áramerősségre, ha az MCP4728 a
#       legmagasabb feszültségen volt (2,048 V), majd a 'channel_x'
#       paraméterek megadhatók a léptető kívánt amperértékével.
#       Az alapértelmezett az, hogy a 'channel_x'
#       paramétereket nem méretezzük.
```
...
[display]
lcd_type:
#       Állítsa az „ssd1306” vagy az „sh1106” értékre az adott megjelenítési
#       típushoz.
#i2c_mcu:
#i2c_bus:
#i2c_software_scl_pin:
#i2c_software_sda_pin:
#i2c_speed:
#       Opcionális paraméterek állnak rendelkezésre az I2C buszon keresztül
#       csatlakoztatott kijelzőkhöz.
#       A fenti paraméterek leírását az "általános I2C-beállítások" részben találod.
#cs_pin:
#dc_pin:
#spi_speed:
#spi_bus:
...
#       SPI-beállítások” részben találod.
#       Az alapértelmezett az I2C mód használata a kijelzőhöz.
#reset_pin:
#       A kijelzőn megadható egy alaphelyzetbe állító tű. Ha nincs megadva,
#       akkor a hardvernek rendelkeznie kell egy felhúzással a megfelelő LCD soron.
#contrast:
#       A beállítandó kontraszt. Az érték 0 és 256 között változhat.
#       Az alapértelmezett érték pedig 239.
#vcomh: 0
#       Állítsd be a Vcomh értéket a kijelzőn. Ez az érték egyes OLED-kijelzők
#       "elfolyási" hatásával jár. Az érték 0 és 63 között változhat.
#       Az alapértelmezett érték 0.
#invert: False
...
Ez egy 24 bites alacsony mintavételi sebességű chip, amely "bit-bang" kommunikációt használ. Nyomtatószál mérlegekhez alkalmas.
...
Ez a HX711 négyszer nagyobb mintavételezési sebességű változata, amely alkalmas mérésre.
...
Az ADS1220 egy 24 bites ADC, amely akár 2 khz-es mintavételi frekvenciát is támogat, amely szoftveresen konfigurálható.
...
[sx1509 my_sx1509]
i2c_address:
#       Ez a bővítő által használt I2C cím. A hardveres jumperektől
...
#       a mikrovezérlő busz nevét.
#       Az alapértelmezett érték a mikrovezérlő típusától függ.
#i2c_software_scl_pin:
#i2c_software_sda_pin:
#       Add meg ezeket a paramétereket a mikrovezérlő szoftver alapú
#       I2C "bit-banging" támogatás használatához. A két paraméternek
#       a mikrokontroller két érintkezőjének kell lennie az scl és az sda
>       vezetékeken. Az alapértelmezés az i2c_bus paraméterben
#       meghatározott hardver alapú I2C támogatás használata.
#i2c_speed:
#       Az eszközzel való kommunikáció során használandó I2C sebesség (Hz-ben).
#       A Klipper implementációja a legtöbb mikrovezérlőn 100 000-re van
#       kódolva, és ennek az értéknek nincs hatása.
#       Az alapértelmezett érték 100000.
#       A Linux, az RP2040 és az ATmega támogatja a 400000-et.
```
