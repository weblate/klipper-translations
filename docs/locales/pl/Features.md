...
Klipper ma kilka istotnych cech:
...
* Klipper obsługuje drukarki z wieloma mikrokontrolerami. Na przykład, jeden mikrokontroler może być użyty do sterowania ekstruderem, podczas gdy inny kontroluje grzałki drukarki, a trzeci kontroluje resztę drukarki. Oprogramowanie hosta Klipper implementuje synchronizację zegara, aby uwzględnić dryf zegara pomiędzy mikrokontrolerami. Nie potrzeba specjalnego kodu, aby włączyć wiele mikrokontrolerów - wymaga to tylko kilku dodatkowych linii w pliku konfiguracyjnym.
* Konfiguracja za pomocą prostego pliku konfiguracyjnego. Nie ma potrzeby ponownego programowania mikrokontrolera w celu zmiany ustawień. Cała konfiguracja Klippera jest przechowywana w standardowym pliku konfiguracyjnym, który można łatwo edytować. Ułatwia to konfigurację i konserwację sprzętu.
* Klipper obsługuje „Smooth Pressure Advance” – mechanizm uwzględniający wpływ ciśnienia w wytłaczarce. Zmniejsza to „ściekanie” z końcówki dyszy i poprawia jakość narożników wydruku. Implementacja Klippera nie wprowadza natychmiastowych zmian prędkości wytłaczania, co poprawia ogólną stabilność i solidność wydruków.
* Klipper obsługuje funkcję „Input Shaping”, aby zmniejszyć wpływ wibracji na jakość druku. Może to zmniejszyć lub wyeliminować „ringing” (znane również jako „duch”, „echo” lub „falowanie”) na wydrukach. Może również pozwolić na uzyskanie większej prędkości drukowania przy jednoczesnym zachowaniu wysokiej jakości druku.
* Klipper wykorzystuje "iteracyjny solver" do obliczania dokładnych czasów kroków na podstawie prostych równań kinematycznych. Ułatwia to przenoszenie Klippera na nowe typy robotów i utrzymuje precyzyjny czas nawet przy złożonej kinematyce (nie jest wymagana "segmentacja linii").
